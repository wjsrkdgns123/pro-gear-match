      ...
      VitePWA({
        ...
        workbox: {
          ...
          navigateFallbackDenylist: [
            ...
            /^\/robots\.txt/,
            /^\/__admin_migrate/,
            // CRITICAL: /en/* and /ko/* navigations must hit the network
            // directly. Their per-language SPA shells are generated by
            // scripts/generate_route_html.mjs AFTER vite build, so workbox's
            // precache only contains the root /index.html (the redirect
            // shell). Without this denylist, SW would serve the redirect
            // shell for /en/ requests, leaving users stuck.
            /^\/en(\/|$)/,
            /^\/ko(\/|$)/,
          ],
